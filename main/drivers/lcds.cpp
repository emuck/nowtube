...
// LEDC backlight: active-low, 8-bit resolution.  duty=0 → GPIO low → max brightness.
constexpr uint32_t LEDC_FREQ_HZ   = 5000;
constexpr uint32_t LEDC_DUTY_OFF  = 256;   // GPIO high → backlight off

static spi_device_handle_t spi_device_handle = nullptr;
static int s_active_lcd_index = -1;  // -1 = none registered
...
void lcds_init() {
  if (initialized) {
    ESP_LOGW(TAG, "lcds_init called more than once — ignoring");
    return;
  }
  ...
  // Backlight via LEDC PWM (active-low: duty=0 → GPIO low → full brightness)
...
void lcds_set_brightness(uint8_t percent) {
  if (percent > 100) percent = 100;
  s_brightness_pct = percent;
  // Active-low: duty=0 → GPIO low → max brightness; duty=256 → GPIO high → off
  uint32_t duty = ((100 - percent) * 256) / 100;
  ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0, duty);
  ledc_update_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0);
}

uint8_t lcds_get_brightness() { return s_brightness_pct; }
...
void lcd_select(size_t index) {
  if (index >= NUM_LCDS) {
    ESP_LOGE(TAG, "lcd_select: index %u out of range", static_cast<unsigned>(index));
    return;
  }
  if (!initialized) {
    ESP_LOGE(TAG, "lcd_select called before lcds_init");
    return;
  }
  // Short-circuit: avoid the remove/add round-trip when the same display is
  // selected consecutively (common during LVGL multi-flush sequences).
  if (s_active_lcd_index == static_cast<int>(index)) {
    return;
  }

  deselect_all_displays();

  if (spi_device_handle != nullptr) {
  ...
  s_active_lcd_index = static_cast<int>(index);
}
...
void lcd_blit_rect(int x, int y, int width, int height, const uint16_t *pixels,
  ...
  if (pixels_size_bytes != static_cast<size_t>(width * height) * sizeof(uint16_t)) {
    ESP_LOGE(TAG, "lcd_blit_rect: pixel buffer size mismatch");
    return;
  }
