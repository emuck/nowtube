...
constexpr uint8_t LED_MAX_BRIGHTNESS = 200; // vendor hardware cap, do not exceed
...
struct leds_state {
  ...
  void set_rgb(size_t index, uint8_t red, uint8_t green, uint8_t blue) {
    ...
    pixel_grb[index * 3 + 0] = green > LED_MAX_BRIGHTNESS ? LED_MAX_BRIGHTNESS : green;
    pixel_grb[index * 3 + 1] = red   > LED_MAX_BRIGHTNESS ? LED_MAX_BRIGHTNESS : red;
    pixel_grb[index * 3 + 2] = blue  > LED_MAX_BRIGHTNESS ? LED_MAX_BRIGHTNESS : blue;
  }
...
// Wait up to timeout_ms for RMT free, then send. Returns true if update was sent.
